use crate::try_create_cln_backend_implementation;
use anyhow::anyhow;
...
use switchgear_components::pool::cln::grpc::client::TonicClnGrpcClient;
...
async fn create_cln_tonic_client(
    credentials: &LnCredentials,
) -> anyhow::Result<Box<dyn LnRpcClient<Error = LnPoolError> + Send + Sync + 'static>> {
    let _ = rustls::crypto::aws_lc_rs::default_provider()
        .install_default()
        .map_err(|_| anyhow!("failed to stand up rustls encryption platform"));

    let backend = try_create_cln_backend_implementation(credentials)?;
...
async fn test_cln_tonic_invoice_with_direct_description() {
    ...
    let client = create_cln_tonic_client(&credentials).await.unwrap();
