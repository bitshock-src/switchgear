...
#[tokio::test]
async fn test_complete_persistence_lifecycle_file_sqlite() {
    let server1 = "server1";
    ...
    let config_path = manifest_dir.join("config/mixed-persistence.yaml");
    ctx.add_server(
        server1,
        config_path,
        Protocol::Https,
...
#[tokio::test]
async fn test_backend_data_loss_with_offer_persistence_file_sqlite() {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let feature_test_config_path = manifest_dir.join(FEATURE_TEST_CONFIG_PATH);
    let mut ctx = match GlobalContext::create(&feature_test_config_path).expect("assert") {
        Some(ctx) => ctx,
