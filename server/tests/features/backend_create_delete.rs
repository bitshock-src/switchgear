...
async fn test_complete_backend_lifecycle_management() {
    ...
    step_and_the_payee_has_created_an_offer_linked_to_both_lightning_nodes(&mut ctx)
        .await
        .expect("assert");
    step_and_both_nodes_are_ready_to_be_registered_as_backends(&mut ctx)
        .await
        .expect("assert");
    ...
    // 2. When the admin deletes the first backend
    step_when_the_admin_deletes_the_first_backend(&mut ctx)
        .await
        .expect("assert");
    ...
    // 4. When the admin deletes the second backend
    step_when_the_admin_deletes_the_second_backend(&mut ctx)
        .await
        .expect("assert");
