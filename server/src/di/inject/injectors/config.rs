...
impl ServerConfigInjector {
    pub fn new(config_path: PathBuf) -> Result<Self, DiError> {
        info!("loading configuration file: {config_path:?}");

        let config_content = std::fs::read_to_string(&config_path).with_foreign_context(
            || {
                format!(
        ...
        let expanded_config = shellexpand::env(&config_content).with_foreign_context(
            || {
                format!(
        ...
        let config: ServerConfig = serde_saphyr::from_str(&expanded_config).with_foreign_context(
            || {
                format!(
