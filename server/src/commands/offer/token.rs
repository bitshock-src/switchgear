...
use switchgear_service::{OfferAudience, OfferBearerTokenValidator, OfferClaims};

fn token(expires: u64) -> Result<OfferClaims, CliError> {
    Ok(OfferClaims {
...
pub fn verify(
    ...
    let validator = OfferBearerTokenValidator::new(public_key);
    let token = validator
        .validate_token(&token)
        .foreign_context("validating offer token", None)?;
    let token = serde_json::to_string_pretty(&token)
        .foreign_context("serializing offer token", ErrorOrigin::Internal)?;
